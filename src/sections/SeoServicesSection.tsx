...
export default function SeoServicesSection() {
  ...
  return (
    // ✅ Full-bleed section; clip decorative abs elements
    <section id="advanced-seo" className="relative w-full overflow-hidden scroll-mt-24 py-20">
      {/* ✅ Clipped, responsive background so it can't cause horizontal scroll */}
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute left-1/2 top-[-25%] h-[720px] w-[min(1100px,100vw)] -translate-x-1/2 rounded-full bg-gradient-to-tr from-indigo-600/25 via-cyan-400/15 to-amber-300/10 blur-3xl" />
      </div>

      {/* ✅ Constrained inner wrapper (was on <section>) */}
      <div className="mx-auto max-w-7xl px-4">
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((it) => (
            <article key={it.title} className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur transition hover:bg-white/10">
        ...
        {/* Tech chips */}
        <div className="mt-10 rounded-2xl border border-white/10 bg-slate-900/40 p-4 backdrop-blur">
          <div role="list" aria-label="Technologies we use" className="flex flex-wrap items-center gap-3 text-xs text-slate-200">
            {[
              "Next.js App Router",
              ...
              <span key={t} role="listitem" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/30 px-3 py-1">
